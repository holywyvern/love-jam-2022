...
function Message_Manager:setup()
  ...
  self._bop = 1
  self._visible = false
  self._options = { showFrame = false, bottom = true }
end
...
function Message_Manager:draw()
  if self._visible then
    ...
    if self._text:is_finished() then
      ...
      love.graphics.draw(Assets.graphics.system.cursor, (width - 16) / 2, y + 80 - 18 + bop)
    end
  end
end
...
function Message_Manager:show(text, options)
  ...
  self._options.bottom = options.bottom or false
end
